...
declare global {
	var __tikzConsoleWriter: ((message: string) => void) | undefined;
}
...
const deferredPromise = (): DeferredPromise<void> => {
...
const writeSync = (file, buffer, pointer = 0, length = buffer.length - pointer) => {
...
const writeToConsole = (x) => {
    ...
    if (consoleBuffer.indexOf('\n') >= 0) {
        ...
        for (const line of lines) {
            ...
            if (typeof globalThis.__tikzConsoleWriter === 'function') {
                globalThis.__tikzConsoleWriter(line);
            } else if (typeof postMessage === 'function') {
                postMessage({ type: 'console', message: line });
            }
        }
    }
};
